// app.js - Express server for Service Mesh Configuration Security Analyzer
...
class BaseSecurityAnalyzer {
  ...
  getNISTControls() {
    // NIST 800-53 Rev 5 Controls - focused on relevant security controls for service meshes
    return {
      "AC-3": {
        id: "AC-3",
...
class ConsulSecurityAnalyzer extends BaseSecurityAnalyzer {
  constructor() {
    super();
    // Flag to indicate whether we're evaluating for FedRAMP compliance
  ...
  checkFedRAMPCompliance(config) {
    // Check TLS certificate expiration (FedRAMP requires shorter periods)
    const connect = config.connect || {};
    const caConfig = connect.ca_config || {};
    ...
    // Check minimum TLS version (FedRAMP requires TLS 1.2 or higher)
  ...
  try {
    ...
    try {
      if (fileExt === '.json') {
        ...
        // For the purpose of this example, we'll handle HCL as if it were parsed
        // In a real implementation, you would use an HCL parser like hcl-parser
        return res.status(400).json({
          success: false,
          message: 'HCL parsing not fully implemented. Please convert to JSON format.'
        });
      } else {
        // Default to YAML for all other extensions
        config = yaml.load(fileContent);
      }
    } catch (e) {
      return res.status(400).json({
        success: false,
        message: `Invalid file format (${fileExt}): ${e.message}`
      });
    }
    
    // Determine mesh type and select appropriate analyzer
    if (config.kind === 'MeshConfig' && config.apiVersion && config.apiVersion.includes('istio.io')) {
      meshType = 'Istio';
      analyzer = new IstioSecurityAnalyzer();
    } else if (config.mesh_type === 'consul') {
      meshType = 'Consul';
      analyzer = new ConsulSecurityAnalyzer();
    } else if (config.mesh_type === 'linkerd') {
      meshType = 'Linkerd';
      analyzer = new LinkerdSecurityAnalyzer();
    } else {
      // Try to auto-detect based on configuration patterns
      if (config.connect && config.tls && config.acl) {
        meshType = 'Consul (auto-detected)';
        // Add mesh_type for analyzer validation
        config.mesh_type = 'consul';
        analyzer = new ConsulSecurityAnalyzer();
      } else if (config.meshMTLS || config.defaultConfig || config.peerAuthentication) {
        meshType = 'Istio (auto-detected)';
        // Add kind/apiVersion for analyzer validation
        config.kind = 'MeshConfig';
        config.apiVersion = 'networking.istio.io/v1alpha1';
        analyzer = new IstioSecurityAnalyzer();
      } else if (config.identity && config.proxy && (config.tls || config.policy)) {
        meshType = 'Linkerd (auto-detected)';
        // Add mesh_type for analyzer validation
        ...
        return res.status(400).json({
          success: false,
          message: 'Could not determine mesh type. Please ensure the file is a valid service mesh configuration (Istio, Consul, or Linkerd).'
        });
      }
    }
...
if (require.main === module) {
  app.listen(port, () => {
    console.log(`Service Mesh Security Analyzer running at http://localhost:${port}`);
  });
}
...
class LinkerdSecurityAnalyzer extends BaseSecurityAnalyzer {
  ...
  checkIdentity(config) {
    ...
    if (identity.certValidityPeriod && identity.certValidityPeriod > 8760) { // More than a year in hours
      this.addFinding(
        'Medium', 
        'Certificate Security', 
...
module.exports = { IstioSecurityAnalyzer, ConsulSecurityAnalyzer, LinkerdSecurityAnalyzer, app };
